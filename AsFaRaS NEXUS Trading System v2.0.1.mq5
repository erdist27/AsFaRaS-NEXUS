//+------------------------------------------------------------------+
//|                AsFaRaS NEXUS Trading System v2.0.1               |
//|         "Beş Sembol. Sonsuz Döngü. Sıfır Zarar."                |
//|                    — AsFaRaS NEXUS —                             |
//+------------------------------------------------------------------+
#property copyright   "AsFaRaS NEXUS System v2.0.1"
...
#define CLR_NEON_BORDER2       C'0,60,60'
...
#define CLR_NEON_DIM           C'70,80,110'
...
//| INPUT                                                             |
//+------------------------------------------------------------------+
input group "╔═══════ AsFaRaS NEXUS v2.0.1 ═══════╗"
...
string g_symbolBases[5][8] = {
   {"XAUUSD","GOLD","XAU","GLD","gold","Gold","xauusd",""},
   {"XAGUSD","SILVER","XAG","SLV","silver","Silver","xagusd",""},
   {"EURUSD","EUR","eurusd","eur","","","",""},
   {"BTCUSD","BITCOIN","BTC","Bitcoin","btc","btcusd","",""},
   {"ETHUSD","ETHEREUM","ETH","Ethereum","eth","ethusd","",""}
};
...
void AddToLogBuffer(string message, color clr) {
   ...
   if(g_logBuffer.count < LOG_BUFFER_SIZE) g_logBuffer.count++;
}
...
void NexusLog(ENUM_LOG_LEVEL level, string symbol, string message) {
   ...
   Print(logLine);
   string shortLine = StringFormat("%s [%s] %s", timeStr, symbol, message);
...
string ResolveSymbol(int symIdx) {
   ...
   // Chart sembolü ile eşleş
   for(int b = 0; b < 8; b++) {
      if(g_symbolBases[symIdx][b] == "") continue;
      string base = g_symbolBases[symIdx][b];
      // Tam eşleşme veya içerme
      if(chartSym == base ||
         StringFind(chartSym, base) == 0 ||
         StringFind(base, chartSym) == 0) {
         if(SymbolInfoDouble(chartSym, SYMBOL_BID) > 0)
            return chartSym;
      }
   }
   // Broker'da tüm kombinasyonları dene
   for(int b = 0; b < 8; b++) {
      if(g_symbolBases[symIdx][b] == "") continue;
      for(int s = 0; s < BROKER_SUFFIX_COUNT; s++) {
         string candidate = g_symbolBases[symIdx][b] + g_brokerSuffixes[s];
   ...
   NexusLog(LOG_WARNING, "SYMRES",
      StringFormat("[%d] Fallback: %s", symIdx, g_symbolBases[symIdx][0]));
...
int DetectChartSymbolIndex() {
   ...
   return -1; // Bilinmeyen sembol — panel modu kullan
}

//+------------------------------------------------------------------+
//| MAGIC ID — Deterministik                                         |
//+------------------------------------------------------------------+
...
void SendTelegramMessage(string message) {
   ...
   if(res == -1) Print("Telegram hata:", GetLastError());
}
...
void SendHourlyReport() {
   ...
   string symInfo = "";
   for(int i = 0; i < NEXUS_SYMBOLS_COUNT; i++) {
      symInfo += StringFormat("%s: H=%.0f P=%+.2f$\n",
                 GetSymbolShortName(i),
                 g_marketData[i].healthScore,
                 GetPacketPnL(i));
   }
   string msg = StringFormat(
      "NEXUS v2.0.1 SAATLIK RAPOR\n%s\n\n"
      "Bakiye: $%.2f | Equity: $%.2f\n\n"
...
void SendDailyReport() {
   ...
   string icon   = daily >= 0 ? "[+]" : "[-]";
   string msg = StringFormat(
      "NEXUS v2.0.1 GUNLUK RAPOR\n%s\n\n"
      "%s Gunluk P&L: %+.2f$\nBakiye: $%.2f\n"
      "Max DD: %.2f%%\nRecovery: %d\nAI: %d ornek\n\n"
      "%s\n-- AsFaRaS NEXUS --",
      TimeToString(TimeCurrent(), TIME_DATE),
      icon, daily, bal, g_stats.maxDrawdownReached,
      g_stats.recoveryCount, g_memoryCount, g_currentQuote);
   SendTelegramMessage(msg);
}
...
// FIX: long → double dönüşüm uyarısı giderildi (Satır 792)
void UpdateAverageVolume(int symIdx) {
   double curVol = (double)iVolume(g_resolvedSymbols[symIdx], PERIOD_M5, 1);
   double alpha  = 0.1;
   if(g_marketData[symIdx].avgVolume <= 0)
      g_marketData[symIdx].avgVolume = curVol;
   else
      g_marketData[symIdx].avgVolume =
         alpha * curVol + (1.0 - alpha) * g_marketData[symIdx].avgVolume;
   g_marketData[symIdx].currentVolume = curVol;
}

bool HasSufficientVolume(int symIdx) {
   double curVol = (double)iVolume(g_resolvedSymbols[symIdx], PERIOD_M1, 0);
...
int GetCurrentDayOfWeek() {
   MqlDateTime dt; TimeToStruct(TimeGMT(), dt); return dt.day_of_week;
}
...
double CalcVolumeFactor(int symIdx) {
   double totalVol = 0; int counted = 0;
   for(int i = 1; i <= 5; i++) {
      double v = (double)iVolume(g_resolvedSymbols[symIdx], PERIOD_M5, i);
      if(v > 0) { totalVol += v; counted++; }
   }
   double avgVol = counted > 0 ? totalVol / counted : 0;
   double curVol = (double)iVolume(g_resolvedSymbols[symIdx], PERIOD_M5, 0);
   if(avgVol <= 0 || curVol <= 0) return 12.5;
   double ratio = MathMax(0.1, MathMin(3.0, curVol / avgVol));
   double score;
...
void UpdateAverageSpread(int symIdx) {
   ...
   if(g_marketData[symIdx].avgSpread > 0)
      g_marketData[symIdx].spreadRatio = cur / g_marketData[symIdx].avgSpread;
}
...
void UpdateAIModel() {
   if(g_memoryCount < InpAIMinSamples) { g_aiModel.isReady = false; return; }
   double successHealth = 0;
   int    successCount  = 0;
   double sessionSuccess[24], sessionTotal[24];
   double daySuccess[7], dayTotal[7];
   ArrayInitialize(sessionSuccess, 0); ArrayInitialize(sessionTotal, 0);
   ArrayInitialize(daySuccess,     0); ArrayInitialize(dayTotal,     0);
   ...
   for(int h = 0; h < 24; h++)
      if(sessionTotal[h] >= 3)
         g_aiModel.sessionWeights[h] = sessionSuccess[h] / sessionTotal[h];
   for(int d = 0; d < 7; d++)
      if(dayTotal[d] >= 3)
         g_aiModel.dayWeights[d] = daySuccess[d] / dayTotal[d];
   g_aiModel.overallWinRate = (double)successCount / (double)g_memoryCount;
   g_aiModel.totalSamples   = g_memoryCount;
   g_aiModel.isReady        = true;
   NexusLog(LOG_INFO, "AI",
      StringFormat("Model güncellendi. Ornek:%d WR:%.1f%%",
      g_memoryCount, g_aiModel.overallWinRate*100));
}
...
void RecordTradeResult(int symIdx, double pnl, bool success) {
   ...
   if(g_memoryCount >= MAX_MEMORY_RECORDS) {
      ArrayCopy(g_tradeMemory, g_tradeMemory, 0, 1, MAX_MEMORY_RECORDS-1);
      g_memoryCount = MAX_MEMORY_RECORDS-1;
   }
   TradeMemoryRecord rec; ZeroMemory(rec);
   ...
   rec.duration      = (double)(TimeCurrent()-g_packets[symIdx].openTime)/60.0;
...
double GetAIConfidenceScore(int symIdx) {
   ...
   double score  = 0.5;
   double health = g_marketData[symIdx].healthScore;
   if(health >= g_aiModel.minSuccessHealth) score += 0.20;
   else score -= 0.20 * (1.0 - health / MathMax(1,g_aiModel.minSuccessHealth));
...
string GetAIStatusString(int symIdx) {
   ...
   if(!g_aiModel.isReady)
      return StringFormat("AI:Ogr(%d/%d)", g_memoryCount, InpAIMinSamples);
   double conf = GetAIConfidenceScore(symIdx);
   string rec = (conf >= 0.75) ? "GIRIS" :
                (conf >= 0.60) ? "IZLE " : "BEKLE";
...
void LoadAIMemory() {
   ...
   if(f == INVALID_HANDLE) return;
   if(!FileIsEnding(f)) FileReadString(f); // Başlık atla
   g_memoryCount = 0;
   ...
   if(g_memoryCount > 0) {
      NexusLog(LOG_INFO, "AI",
         StringFormat("AI hafizasi yuklendi: %d kayit", g_memoryCount));
      UpdateAIModel();
   }
}
...
ulong OpenOrder(string symbol, ENUM_ORDER_TYPE type,
   ...
   int slipPoints = point>0 ?
      (int)MathMin(maxSlip, MathMax(3,(spread*MAX_SLIPPAGE_MULT)/point)) : 3;
   ...
   for(int retry = 0; retry < MAX_RETRY_COUNT; retry++) {
      if(OrderSend(request, result) &&
         ...
         NexusLog(LOG_INFO, symbol,
            ...
            type==ORDER_TYPE_BUY?"BUY":"SELL", lot, (int)result.deal));
         return result.deal;
      }
      Sleep(500);
...
bool CloseAllPacketOrders(int symIdx, string reason) {
   ...
   if(allOK) {
      RecordTradeResult(symIdx, totalPnL, totalPnL >= 0);
...
bool CheckLossLimits() {
   ...
   if(drawdown >= InpMaxDrawdown) {
      g_emergencyStop = true; return false;
   }
   double dailyLimit = balance * (InpDailyLossLimit/100.0);
   if(g_account.dailyPnL < -dailyLimit) return false;
   return true;
}
...
bool CheckMarginSafety(int symIdx) {
   ...
   double balance    = AccountInfoDouble(ACCOUNT_BALANCE);
   if(balance <= 0) return false;
   if(freeMargin < balance * 0.20) return false;
   return true;
}
...
bool OpenNormalPacket(int symIdx) {
   ...
   if(g_marketData[symIdx].healthScore < InpMinHealthScore) {
      g_systemStates[symIdx] = STATE_PAUSED; return false;
   }
...
bool OpenBoostX3(int symIdx) {
   string symbol = g_resolvedSymbols[symIdx];
   ulong  magic  = g_magicIDs[symIdx];
   if(g_marketData[symIdx].healthScore < GetBoostHealthRequirement(3)) return false;
   double boostLot = NormalizeLot(symbol, g_packets[symIdx].baseLot*3.0);
   ...
   ulong boostT = OpenOrder(symbol, boostType, boostLot, "ANX_BST_X3", magic);
   if(boostT > 0) {
...
bool OpenBoostX9(int symIdx) {
   ...
   if(PositionSelectByTicket(prevBoost)) {
      ...
      boostType = (pt==POSITION_TYPE_BUY) ? ORDER_TYPE_SELL : ORDER_TYPE_BUY;
   }
   if(prevBoost>0 && !CloseOrder(symbol, prevBoost, "x3_x9")) return false;
   ulong boostT = OpenOrder(symbol, boostType, boostLot, "ANX_BST_X9", magic);
   if(boostT > 0) {
      g_packets[symIdx].boostTicket = (double)boostT;
...
bool OpenBoostX27(int symIdx) {
   ...
   if(!OrderCalcMargin(ORDER_TYPE_BUY, symbol, boostLot,
      SymbolInfoDouble(symbol,SYMBOL_ASK), reqMargin)) return false;
   double freeMargin = AccountInfoDouble(ACCOUNT_MARGIN_FREE);
   if(freeMargin < reqMargin * 1.5) return false;
   if(!CheckLossLimits()) return false;
   ulong  prevBoost = (ulong)g_packets[symIdx].boostTicket;
   ENUM_ORDER_TYPE boostType = ORDER_TYPE_BUY;
   if(PositionSelectByTicket(prevBoost)) {
   ...
   if(prevBoost>0 && !CloseOrder(symbol, prevBoost, "x9_x27")) return false;
   ulong boostT = OpenOrder(symbol, boostType, boostLot, "ANX_BST_X27", magic);
   if(boostT > 0) {
      g_packets[symIdx].boostTicket = (double)boostT;
...
void CheckBoostTrigger(int symIdx) {
   ...
   if(totalPnL < -threshold) OpenBoostX3(symIdx);
}
...
void CheckBoostConditions(int symIdx) {
   ...
   if(PositionSelectByTicket(boostTkt)) {
      ...
      boostPosType = (ENUM_POSITION_TYPE)PositionGetInteger(POSITION_TYPE);
   }
   double buyPnL=0, sellPnL=0;
   if(PositionSelectByTicket(buyTkt))  buyPnL  = PositionGetDouble(POSITION_PROFIT);
   if(PositionSelectByTicket(sellTkt)) sellPnL = PositionGetDouble(POSITION_PROFIT);
   double mainPnL  = buyPnL + sellPnL;
   int    digits   = g_marketData[symIdx].digits;
   double pipSize  = (digits==3||digits==5) ? point*10 : point;
   ...
   if(bs == BOOST_X3_ACTIVE) {
      ...
      if(totalPnL >= targetProfit) {
         CloseOrder(symbol,(ulong)g_packets[symIdx].buyTicket, "BST_OK");
         CloseOrder(symbol,(ulong)g_packets[symIdx].sellTicket,"BST_OK");
         g_packets[symIdx].boostState = BOOST_COMPLETED;
         g_systemStates[symIdx]       = STATE_SCANNING;
         g_account.successPackets++;
         InitTrailingState(symIdx, boostTkt, targetProfit);
         RecordTradeResult(symIdx, totalPnL, true); return;
      }
      ...
      double totalPnL = mainPnL + boostPnL;
      if(totalPnL >= targetProfit) {
         CloseOrder(symbol,(ulong)g_packets[symIdx].buyTicket, "x9_OK");
         CloseOrder(symbol,(ulong)g_packets[symIdx].sellTicket,"x9_OK");
         g_packets[symIdx].boostState = BOOST_COMPLETED;
         g_systemStates[symIdx]       = STATE_SCANNING;
         g_account.successPackets++;
         RecordTradeResult(symIdx, totalPnL, true); return;
      ...
      if(totalPnL >= 0) {
         ...
         g_account.successPackets++; return;
      }
...
double GetATRTrailingStep(int symIdx) {
   ...
   double pipSize = (g_marketData[symIdx].digits==3||
                    g_marketData[symIdx].digits==5) ? point*10 : point;
...
double CalculateSafeSL(string symbol, double price,
   ...
   return NormalizeDouble(safeSL, (int)SymbolInfoInteger(symbol,SYMBOL_DIGITS));
}

bool IsValidSLDistance(string symbol, double price, double sl, bool isBuy) {
   int    stopLvl = (int)SymbolInfoInteger(symbol, SYMBOL_TRADE_STOPS_LEVEL);
   double point   = SymbolInfoDouble(symbol, SYMBOL_POINT);
...
bool ApplyBreakEven(int symIdx, bool isBuy, double curPrice) {
   string symbol = g_resolvedSymbols[symIdx];
   double spread = GetCurrentSpread(symbol)*SymbolInfoDouble(symbol,SYMBOL_POINT);
   ...
   if(UpdatePositionSL(symIdx, ticket, newSL)) {
      ...
      NexusLog(LOG_INFO, symbol, StringFormat("BREAKEVEN SL:%.5f", newSL));
      return true;
   }
   return false;
}
...
void UpdateContinuousTrailing(int symIdx, bool isBuy,
   ...
   double newSL; bool shouldUpdate = false;
   if(isBuy) {
      newSL = curPrice - trailStep;
...
void UpdateTrailing(int symIdx) {
   ...
   if(!PositionSelectByTicket(ticket)) {
      ...
      g_trailingStates[symIdx].level    = TRAIL_COMPLETED; return;
   }
   string symbol = g_resolvedSymbols[symIdx];
   ...
   if(!g_trailingStates[symIdx].level4Hit && pct>=l4) {
      if(ApplyTrailingLevel(symIdx,75.0,TRAIL_LEVEL_4,isBuy,curPrice,curProfit)) {
         g_trailingStates[symIdx].level4Hit=true; g_trailingStats.level4Hits++; } return;
   }
   if(!g_trailingStates[symIdx].level3Hit && pct>=l3) {
      if(ApplyTrailingLevel(symIdx,50.0,TRAIL_LEVEL_3,isBuy,curPrice,curProfit)) {
         g_trailingStates[symIdx].level3Hit=true; g_trailingStats.level3Hits++; } return;
   }
   if(!g_trailingStates[symIdx].level2Hit && pct>=l2) {
      if(ApplyTrailingLevel(symIdx,25.0,TRAIL_LEVEL_2,isBuy,curPrice,curProfit)) {
         g_trailingStates[symIdx].level2Hit=true; g_trailingStats.level2Hits++; } return;
   }
   if(!g_trailingStates[symIdx].level1Hit && pct>=l1) {
      if(ApplyBreakEven(symIdx,isBuy,curPrice))
         g_trailingStates[symIdx].level1Hit=true; return;
   }
...
string SelectMotivationQuote() {
   ...
   if(g_aiModel.isReady && g_currentQuoteIdx%4==0) {
      idx=g_currentQuoteIdx%ArraySize(g_quotesAI); return g_quotesAI[idx];
   }
...
// Objeyi sil (yeniden boyutlandırma gerektiğinde)
...
void DrawMiniDashboard(int symIdx) {
   ...
   NeonLabel(DASH_PREFIX+"M_LIV", "LIVE", x+w-50, y+8, 8, liveClr);

   // Saat
   if(InpShowClock)
      NeonLabel(DASH_PREFIX+"M_CLK", GetDigitalClock(),
                x+w-120, y+8, 9, CLR_NEON_CYAN);
   ...
   NeonLabel(DASH_PREFIX+"M_HLB",
      ...
      x+8, cy, 8, GetHealthColor(health)); cy+=18;
   ...
   NeonLabel(DASH_PREFIX+"M_BAL",
      ...
      x+8, cy, 7,
      g_account.drawdown < 5 ? CLR_NEON_GREEN : CLR_NEON_ORANGE);
   cy+=16;

   // Motivasyon
   UpdateMotivationQuote();
   string shortQ = g_currentQuote;
   if(StringLen(shortQ) > 42)
      shortQ = StringSubstr(shortQ, 0, 40) + "..";
...
//+------------------------------------------------------------------+
//| MOD B: FULL NEON PANEL (MODE_PANEL) — FIX: Her zaman çizilir   |
//+------------------------------------------------------------------+
void DrawFullNeonPanel() {
   // Chart boyutlarını al
   int chartW = (int)ChartGetInteger(0, CHART_WIDTH_IN_PIXELS);
   int chartH = (int)ChartGetInteger(0, CHART_HEIGHT_IN_PIXELS);

   // Boyut sıfırsa çizme (chart henüz hazır değil)
   if(chartW <= 100 || chartH <= 100) return;
   ...
   NeonRect(DASH_PREFIX+"F_TOP", px, py, pw, 2,  CLR_NEON_BORDER);
   NeonRect(DASH_PREFIX+"F_BOT", px, ph-2, pw, 2, CLR_NEON_BORDER);
   NeonRect(DASH_PREFIX+"F_LFT", px, py, 2, ph, CLR_NEON_BORDER);
   NeonRect(DASH_PREFIX+"F_RGT", pw-2, py, 2, ph, CLR_NEON_BORDER);

   // ── HEADER (0-56) ──
   NeonRect(DASH_PREFIX+"F_HDR", px, py, pw, 56, CLR_NEON_HEADER, CLR_NEON_BORDER);

   NeonLabel(DASH_PREFIX+"F_NM1", "AsFaRaS NEXUS",
             px+16, py+5, 20, CLR_NEON_CYAN);
   NeonLabel(DASH_PREFIX+"F_NM2", "v"+NEXUS_VERSION,
             px+280, py+12, 8, CLR_NEON_DIM);
   NeonLabel(DASH_PREFIX+"F_SLG",
             "Bes Sembol  |  Sonsuz Dongu  |  Sifir Zarar  |  Ajan Konseyi Onayli",
             px+16, py+36, 8, CLR_NEON_PURPLE);

   // Live indikatör
   g_dashCache.liveToggle = !g_dashCache.liveToggle;
   color liveC = g_dashCache.liveToggle ? CLR_NEON_GREEN : C'0,60,30';
   NeonLabel(DASH_PREFIX+"F_LV1", "●", pw-140, py+8,  16, liveC);
   NeonLabel(DASH_PREFIX+"F_LV2", "LIVE", pw-118, py+14, 9, CLR_NEON_GREEN);
   ...
   NeonLabel(DASH_PREFIX+"F_BAL",
      StringFormat("$%.2f", bal),
      pw-220, py+6, 14, eq>=bal ? CLR_NEON_GREEN : CLR_NEON_RED);
   NeonLabel(DASH_PREFIX+"F_EQT",
      StringFormat("EQ:$%.2f", eq),
      pw-220, py+32, 8, CLR_NEON_DIM);

   // ── 3 KOLON ──
   int colY = py + 60;
   int colH = ph - 90;  // Alt şerit için boşluk bırak
   int gap  = 4;
   int colW = (pw - gap*4) / 3;
   int col1x = gap;
   ...
   NeonRect(DASH_PREFIX+"F_C1B", col1x, colY, colW, colH, CLR_NEON_BG2, CLR_NEON_BORDER2);
   NeonRect(DASH_PREFIX+"F_C2B", col2x, colY, colW, colH, CLR_NEON_BG2, CLR_NEON_BORDER2);
   NeonRect(DASH_PREFIX+"F_C3B", col3x, colY, colW, colH, CLR_NEON_BG2, CLR_NEON_BORDER2);

   // ═══════════════════════════════
   // KOLON 1 — 5 Sembol Paneli
   // ═══════════════════════════════
   NeonLabel(DASH_PREFIX+"F_C1T", "  SEMBOL PANELI",
             col1x+6, colY+5, 9, CLR_NEON_CYAN);
   NeonRect(DASH_PREFIX+"F_C1L", col1x+4, colY+20, colW-8, 1, CLR_NEON_BORDER2);

   int sy = colY + 26;
   int rowH = (colH - 30) / NEXUS_SYMBOLS_COUNT;
   ...
   for(int i = 0; i < NEXUS_SYMBOLS_COUNT; i++) {
      ...
      NeonRect(DASH_PREFIX+"F_RB"+si,
         col1x+3, sy, colW-6, rowH-1, rowBg);

      // Sembol + Durum
      NeonLabel(DASH_PREFIX+"F_RS"+si,
         GetSymbolShortName(i) + " " + GetStateString(i),
         col1x+8, sy+2, 9, GetStateColor(i));

      // AI status
      NeonLabel(DASH_PREFIX+"F_RAI"+si,
         GetAIStatusString(i),
         col1x+colW-115, sy+2, 7, CLR_NEON_PURPLE);

      // Health bar
      string hbar = DrawHealthBar(h, 14);
      NeonLabel(DASH_PREFIX+"F_RH"+si,
         StringFormat("[%s] %.0f", hbar, h),
         col1x+8, sy+18, 7, GetHealthColor(h));

      // P&L + Boost
      if(act) {
         color pc = pnl >= 0 ? CLR_NEON_GREEN : CLR_NEON_RED;
         NeonLabel(DASH_PREFIX+"F_RP"+si,
            StringFormat("P&L:%+.2f$  Lot:%.2f  B:%s",
      ...
      NeonLabel(DASH_PREFIX+"F_RT"+si,
         ...
         col1x+8, sy+46, 7, CLR_NEON_BLUE);

      sy += rowH;
   }

   // ═══════════════════════════════
   // KOLON 2 — Merkez Panel
   // ═══════════════════════════════
   int midY = colY + 8;

   // Dijital saat — büyük
   string clockStr = GetDigitalClock();
   NeonLabel(DASH_PREFIX+"F_CLK", clockStr,
             col2x + colW/2 - 58, midY, 26, CLR_NEON_CYAN);
   midY += 52;

   NeonLabel(DASH_PREFIX+"F_DAT", GetDigitalDate(),
             col2x + colW/2 - 60, midY, 9, CLR_NEON_DIM);
   midY += 22;

   NeonRect(DASH_PREFIX+"F_ML1", col2x+6, midY, colW-12, 1, CLR_NEON_BORDER2);
   midY += 10;

   // Hesap Barları
   NeonLabel(DASH_PREFIX+"F_ABT", "  HESAP DURUMU",
             col2x+6, midY, 9, CLR_NEON_CYAN); midY += 18;
   ...
   color ddC = dd<5 ? CLR_NEON_GREEN : dd<10 ? CLR_NEON_YELLOW : CLR_NEON_RED;
   NeonLabel(DASH_PREFIX+"F_DD",
      StringFormat("DD   [%s] %.1f%%", DrawProgressBar(dd,InpMaxDrawdown,16), dd),
      col2x+6, midY, 8, ddC); midY+=16;

   color eqC = eq>=bal ? CLR_NEON_GREEN : CLR_NEON_RED;
   NeonLabel(DASH_PREFIX+"F_EQ",
      StringFormat("EQ   [%s] %.1f%%", DrawProgressBar(MathMin(eqPct,100),100,16), eqPct),
      col2x+6, midY, 8, eqC); midY+=16;

   color wrC = wr>60 ? CLR_NEON_GREEN : wr>40 ? CLR_NEON_YELLOW : CLR_NEON_RED;
   NeonLabel(DASH_PREFIX+"F_WR",
      StringFormat("WR   [%s] %.1f%%", DrawProgressBar(wr,100,16), wr),
      col2x+6, midY, 8, wrC); midY+=16;

   if(g_aiModel.isReady) {
      double aiWr = g_aiModel.overallWinRate*100;
      NeonLabel(DASH_PREFIX+"F_AIW",
         StringFormat("AI   [%s] %.1f%%", DrawProgressBar(aiWr,100,16), aiWr),
         col2x+6, midY, 8, CLR_NEON_PURPLE); midY+=16;
   }

   midY+=4;
   NeonRect(DASH_PREFIX+"F_ML2", col2x+6, midY, colW-12, 1, CLR_NEON_BORDER2);
   midY+=10;

   // İstatistikler özet
   NeonLabel(DASH_PREFIX+"F_ST1",
      StringFormat("Paket:%d  OK:%d  HATA:%d",
                   g_account.totalPackets,
                   g_account.successPackets,
                   g_account.failedPackets),
      col2x+6, midY, 8, CLR_NEON_WHITE); midY+=16;

   NeonLabel(DASH_PREFIX+"F_ST2",
      StringFormat("x3:%d  x9:%d  x27:%d  Rec:%d",
                   g_stats.boostX3Count, g_stats.boostX9Count,
                   g_stats.boostX27Count, g_stats.recoveryCount),
      col2x+6, midY, 8, CLR_NEON_DIM); midY+=16;

   midY+=4;
   NeonRect(DASH_PREFIX+"F_ML3", col2x+6, midY, colW-12, 1, CLR_NEON_BORDER2);
   midY+=10;
   ...
   color motBg  = anyRec ? C'28,8,8' :
                  anyBst ? C'28,16,4' : C'10,8,28';

   int quoteBoxH = 80;
   NeonRect(DASH_PREFIX+"F_QB", col2x+4, midY, colW-8, quoteBoxH, motBg, motClr);

   string catStr = anyRec ? "[RECOVERY]" : anyBst ? "[BOOST]" : "[GUNUN SOZU]";
   NeonLabel(DASH_PREFIX+"F_QT",
      catStr + "  >" + GetNextQuoteCountdown() + "<",
      col2x+10, midY+5, 8, motClr);

   // Quote - satıra böl
   string q = "\"" + g_currentQuote + "\"";
   if(StringLen(q) <= 40) {
      NeonLabel(DASH_PREFIX+"F_Q1", q, col2x+10, midY+22, 8, CLR_NEON_WHITE);
      DeleteObj(DASH_PREFIX+"F_Q2");
   } else {
      int sp = 40;
      while(sp > 10 && StringGetCharacter(q,sp) != ' ') sp--;
      NeonLabel(DASH_PREFIX+"F_Q1", StringSubstr(q,0,sp),   col2x+10, midY+22, 8, CLR_NEON_WHITE);
      NeonLabel(DASH_PREFIX+"F_Q2", StringSubstr(q,sp+1),   col2x+10, midY+38, 8, CLR_NEON_WHITE);
   }
   NeonLabel(DASH_PREFIX+"F_QA", "-- AsFaRaS NEXUS --",
             col2x+colW-160, midY+62, 7, motClr);

   // ═══════════════════════════════
   // KOLON 3 — Log + AI + Trailing
   // ═══════════════════════════════
   int logY = colY + 8;

   NeonLabel(DASH_PREFIX+"F_C3T", "  SISTEM LOGU",
             col3x+6, logY, 9, CLR_NEON_CYAN); logY+=18;
   NeonRect(DASH_PREFIX+"F_C3L1", col3x+4, logY, colW-8, 1, CLR_NEON_BORDER2);
   logY+=6;

   // Log kutucuğu
   int logBoxH = LOG_BUFFER_SIZE * 22 + 8;
   NeonRect(DASH_PREFIX+"F_LBG", col3x+3, logY, colW-6, logBoxH, C'6,8,16', CLR_NEON_BORDER2);

   for(int l = 0; l < LOG_BUFFER_SIZE; l++) {
      int    msgIdx = (g_logBuffer.head - LOG_BUFFER_SIZE + l + LOG_BUFFER_SIZE*2) % LOG_BUFFER_SIZE;
      string msg    = g_logBuffer.messages[msgIdx];
      color  clr    = g_logBuffer.colors[msgIdx];
      if(msg == "") { msg = "..."; clr = CLR_NEON_DIM; }
      if(StringLen(msg) > 38) msg = StringSubstr(msg,0,36)+"..";
      NeonLabel(DASH_PREFIX+"F_LM"+IntegerToString(l),
         msg, col3x+8, logY+4+l*22, 7, clr);
   }
   logY += logBoxH + 10;

   NeonRect(DASH_PREFIX+"F_C3L2", col3x+4, logY, colW-8, 1, CLR_NEON_BORDER2);
   logY+=8;

   // AI Motor
   NeonLabel(DASH_PREFIX+"F_AIT", "  AI MOTOR",
             col3x+6, logY, 9, CLR_NEON_CYAN); logY+=16;

   if(!InpAIEnabled) {
      NeonLabel(DASH_PREFIX+"F_AIS", "AI Motor Devre Disi",
                col3x+6, logY, 8, CLR_NEON_DIM); logY+=16;
   } else if(!g_aiModel.isReady) {
      double pct = (double)g_memoryCount / InpAIMinSamples * 100;
      NeonLabel(DASH_PREFIX+"F_AIL",
         StringFormat("Ogrenme [%s] %d/%d",
                      DrawProgressBar(pct,100,12),
                      g_memoryCount, InpAIMinSamples),
         col3x+6, logY, 8, CLR_NEON_YELLOW); logY+=16;
   } else {
      NeonLabel(DASH_PREFIX+"F_AIR",
         StringFormat("WinRate:%.1f%%  Ornek:%d",
                      g_aiModel.overallWinRate*100, g_memoryCount),
         col3x+6, logY, 8, CLR_NEON_GREEN); logY+=14;
      NeonLabel(DASH_PREFIX+"F_AIH",
         StringFormat("MinH:%.0f  Mod:%s",
                      g_aiModel.minSuccessHealth,
                      InpAIAdvisoryOnly?"ONERI":"AKTIF"),
         col3x+6, logY, 7, CLR_NEON_DIM); logY+=16;
   }

   logY+=4;
   NeonRect(DASH_PREFIX+"F_C3L3", col3x+4, logY, colW-8, 1, CLR_NEON_BORDER2);
   logY+=8;

   // Trailing Stats
   NeonLabel(DASH_PREFIX+"F_TRT", "  TRAILING STATS",
             col3x+6, logY, 9, CLR_NEON_CYAN); logY+=16;
   ...
   NeonLabel(DASH_PREFIX+"F_TR1",
      ...
      col3x+6, logY, 7, CLR_NEON_BLUE); logY+=14;

   NeonLabel(DASH_PREFIX+"F_TR2",
      StringFormat("Kilitli Toplam:+$%.2f",
                   g_trailingStats.totalLockedProfit),
      col3x+6, logY, 8,
      g_trailingStats.totalLockedProfit>0 ? CLR_NEON_GREEN : CLR_NEON_DIM);

   // ── ALT FOOTER ──
   int footY = ph - 26;
   NeonRect(DASH_PREFIX+"F_FT", px, footY, pw, 26, CLR_NEON_HEADER, CLR_NEON_BORDER);
   NeonLabel(DASH_PREFIX+"F_FV",
      "AsFaRaS NEXUS v"+NEXUS_VERSION+
      "  |  Ajan Konseyi Onayli  |  AI:"+IntegerToString(g_memoryCount)+" kayit",
      px+12, footY+6, 8, CLR_NEON_DIM);
   NeonLabel(DASH_PREFIX+"F_FT2",
      GetDigitalDate()+"  "+GetDigitalClock(),
      pw-230, footY+6, 9, CLR_NEON_CYAN);

   ChartRedraw(0);
}

//+------------------------------------------------------------------+
//| DASHBOARD ANA YÖNETİCİ — FIX: Her tick çalışır                 |
//+------------------------------------------------------------------+
void UpdateDashboard() {
   // Her 1 saniyede bir güncelle (saat için gerekli)
   if(TimeCurrent() - g_lastDashUpdate < 1) return;
   g_lastDashUpdate = TimeCurrent();

   if(g_operationMode == MODE_PANEL) {
      // Panel modu: her zaman full panel çiz
      DrawFullNeonPanel();
   }
   else {
      // Symbol veya Auto modu: mini dashboard
...
bool LoadSystemState() {
   ...
   if((ver != 200 && ver != 201) || syms != NEXUS_SYMBOLS_COUNT) {
      NexusLog(LOG_WARNING, "SYSTEM",
         StringFormat("Durum dosyasi uyumsuz v%d", ver));
   ...
   FileClose(f);
   NexusLog(LOG_INFO, "SYSTEM", "Sistem durumu kurtarildi v2.0.1!");
   return true;
}
...
void WriteHeartbeat() {
   ...
   if(f != INVALID_HANDLE) {
      FileWrite(f, TimeToString(TimeCurrent()) + "|" +
                   DoubleToString(g_account.equity,2) + "|v" + NEXUS_VERSION);
      FileClose(f);
   }
}
...
bool IsNewM5Candle(int symIdx) {
   datetime cur = iTime(g_resolvedSymbols[symIdx], PERIOD_M5, 0);
   if(cur != g_lastM5Time[symIdx]) {
      g_lastM5Time[symIdx] = cur; return true;
   }
   return false;
}

bool RunBrokerTest(string symbol) {
   NexusLog(LOG_INFO, "BROKER", "Test: " + symbol);
   bool ok = true;
   if(!SymbolSelect(symbol, true)) ok = false;
   g_broker.hedgeAllowed = (AccountInfoInteger(ACCOUNT_MARGIN_MODE)==
   ...
   g_broker.stopLevel= (int)SymbolInfoInteger(symbol,SYMBOL_TRADE_STOPS_LEVEL);
   if(!(bool)TerminalInfoInteger(TERMINAL_TRADE_ALLOWED)) ok = false;
   g_broker.testPassed = ok;
   NexusLog(ok?LOG_INFO:LOG_WARNING, "BROKER",
      StringFormat("Test:%s Hedge:%s StopLvl:%d",
      ok?"OK":"WARN", g_broker.hedgeAllowed?"OK":"NO",
      g_broker.stopLevel));
   return ok;
}
...
bool InitIndicatorHandles() {
   bool allOK = true;
   for(int i = 0; i < NEXUS_SYMBOLS_COUNT; i++) {
      g_atrHandles[i]   = iATR(g_resolvedSymbols[i], PERIOD_M5, InpHealthPeriod);
      g_atrHandles14[i] = iATR(g_resolvedSymbols[i], PERIOD_M5, 14);
      if(g_atrHandles[i]==INVALID_HANDLE || g_atrHandles14[i]==INVALID_HANDLE) {
         NexusLog(LOG_WARNING, g_resolvedSymbols[i], "ATR handle uyarisi");
         allOK = false;
      }
   }
   return allOK;
...
bool InitializeGlobals() {
   ...
   if(g_operationMode == MODE_AUTO) {
      g_operationMode = (g_singleSymbolIdx >= 0) ? MODE_SYMBOL : MODE_PANEL;
   }

   NexusLog(LOG_INFO, "SYSTEM",
      StringFormat("Mod:%s ChartIdx:%d Chart:%s",
      g_operationMode==MODE_SYMBOL?"SYMBOL":"PANEL",
      g_singleSymbolIdx, Symbol()));

   g_systemStartTime = TimeCurrent();
   ZeroMemory(g_stats);  g_stats.systemStartTime = g_systemStartTime;
   ...
   for(int i = 0; i < NEXUS_SYMBOLS_COUNT; i++) {
      ...
      if(!SymbolSelect(g_resolvedSymbols[i], true)) {
         NexusLog(LOG_WARNING, "INIT",
            StringFormat("Sembol secilemiyor: %s (devam)",
                         g_resolvedSymbols[i]));
         g_systemStates[i] = STATE_ERROR;
         // Devam et, diğer semboller için
...
void ProcessSymbol(int symIdx) {
   ...
   if(state == STATE_SCANNING || state == STATE_RECOVERY) {
      ...
      if(g_marketData[symIdx].healthScore < InpMinHealthScore) {
         g_systemStates[symIdx] = STATE_PAUSED; return;
      }
...
int OnInit() {
   Print("╔══════════════════════════════════════════╗");
   Print("║  AsFaRaS NEXUS Trading System v2.0.1    ║");
   Print("║  Bes Sembol. Sonsuz Dongu. Sifir Zarar. ║");
   Print("╚══════════════════════════════════════════╝");

   OpenLogFile();
   NexusLog(LOG_INFO, "SYSTEM", "AsFaRaS NEXUS v2.0.1 baslatiliyor...");
   ...
   // İlk dashboard çizimi — gecikme olmadan
   ...
   NexusLog(LOG_INFO, "SYSTEM", "AsFaRaS NEXUS v2.0.1 HAZIR!");
...
void OnDeinit(const int reason) {
   ...
   for(int i = 0; i < NEXUS_SYMBOLS_COUNT; i++) {
      ...
      if(g_atrHandles14[i] != INVALID_HANDLE) IndicatorRelease(g_atrHandles14[i]);
   }
   ...
   NexusLog(LOG_INFO,"SYSTEM","Durduruldu:"+reasonStr);
   if(g_logFileHandle!=INVALID_HANDLE) {
      FileWrite(g_logFileHandle,"=== LOG END ===");
      FileClose(g_logFileHandle);
   }
   SendTelegramMessage(StringFormat(
      "NEXUS v%s DURDURULDU\n%s\n%s\n"
      "Paket:%d OK:%d AI:%d kayit\n-- AsFaRaS NEXUS --",
...
void OnTick() {
   ...
   if(g_account.drawdown >= InpMaxDrawdown) {
      NexusLog(LOG_CRITICAL,"SYSTEM",
         StringFormat("EMERGENCY STOP! DD:%.2f%%",g_account.drawdown));
      g_emergencyStop = true;
      SendTelegramMessage("NEXUS EMERGENCY STOP!\nDrawdown limiti!\n-- AsFaRaS NEXUS --");
   ...
   // Dashboard — her tick'te kontrol (1 sn throttle içinde)
   UpdateDashboard();
   WriteHeartbeat();
...
void OnTimer() {
   ...
   // Timer ile de dashboard güncelle (saat animasyonu için)
   UpdateDashboard();
}
...
//+------------------------------------------------------------------+
//|      AsFaRaS NEXUS v2.0.1 — TAMAMLANDI                         |
//|   "Beş Sembol. Sonsuz Döngü. Sıfır Zarar."                     |
//|              — AsFaRaS NEXUS —                                  |
//+------------------------------------------------------------------+
